...
# 0/ Start figure
fig = plt.figure(figsize=(15, 5.35))
...
## MOVED TO SOLVE ZORDER OF (a) LABEL
...
# 2.1/ First, an horizontal axis on top to plot the sample histograms
...
ax1 = fig.add_subplot(grid[2:9, :10])
...
ax1.hlines(0, xmin=-1, xmax=15, color='tab:grey', linestyles='dashed', zorder=1)
...
a = list(ax1.get_children())

# Finally, plot the violin plot
sns.violinplot(ax=ax1, x="slope_category", y="dh", hue="terrain",
...
ax1.set_xlabel('Slope categories (degrees)', labelpad=-0.5)
ax1.set_ylabel('Elevation\ndifferences\n(m)', labelpad=-5)
ax1.set_xlim((-0.5, len(list_counts)-0.5))
...
ax.set_xlabel('Maximum curvature categories (10$^{2}$ m$^{-1}$)', labelpad=-0.5)
ax.set_ylabel('Elevation\ndifferences\n(m)',labelpad=-5)
leg = ax.legend(loc='lower left')
leg.remove()
ax.set_xlim((-0.5, len(list_counts)-0.5))
ax.text(0.5, -0.475, '(b)', transform=ax.transAxes, ha='center', va='top',fontsize=15)
...
ax.set_ylabel(label_var_name_2, labelpad=-0.5)
...
ax.text(0.5, -0.215, '(c)', transform=ax.transAxes, ha='center', va='top',fontsize=15)
...
nodata.text(0.125, 0.35, '$$\le 15 \%$$', ha='left', va='center')
...
nodata.text(0.725, 0.35, '$$30–50 \%$$', ha='left', va='center')
...
nodata.text(0.725, 0.2, '$$\ge 50 \%$$', ha='left', va='center')

nodata.text(0.55, 0.1, 'Dispersion\ndifference\nstable to\nmoving', ha='center', va='top')
nodata.set_xlim((0, 1))
nodata.set_ylim((0, 1))
nodata.set_xticks([])
nodata.set_yticks([])
...
ax.text(0.5, -0.265, '(d)', transform=ax.transAxes, ha='center', va='top', fontsize=15, zorder=30)
...
inside_ax = fig.add_axes([0.76, 0.055, 0.15, 0.15], projection=ccrs.UTM(32), label='subset')
...
# Save to file
plt.savefig('/home/atom/ongoing/work_stderr_dem/figures/final/Figure_4_final.pdf', dpi=400)
