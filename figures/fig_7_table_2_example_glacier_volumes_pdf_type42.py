...
sns.boxplot(ax=ax, x="Glacier area category (km$^{2}$)", y="err_dh", hue="err_category", hue_order=list_name[0:6],
...
ax.set_xlim((-0.5, len(list_gla_nb)-0.5))
ax.text(0.5, -0.175, '(a)', transform=ax.transAxes, ha='center', va='top', fontsize=15,
         zorder=20)
...
list_gla_nb = []
df['Glacier mean slope category (km$^{2}$)'] = ''
for i in range(len(bins_area)-1):
    ind = np.logical_and(df.mean_slope >= bins_slope[i], df.mean_slope < bins_slope[i+1])
    df.loc[ind, ['Glacier mean slope category (km$^{2}$)']] = str(bins_slope[i])+'-'+str(bins_slope[i+1])
    nb_gla = len(np.unique(df[ind].rgiid.values))
    list_gla_nb.append(nb_gla)
df = df.sort_values(by='Glacier mean slope category (km$^{2}$)')
...
sns.boxplot(ax=ax, x="Glacier mean slope category (km$^{2}$)", y="err_dh", hue="err_category", hue_order=list_name[0:6],
                 data=df, palette={list_name[0]:'white', list_name[1]:'lightgrey', list_name[2]:'darkgrey' ,
                                   list_name[3]:'lightgreen', list_name[4]:'green', list_name[5]:'darkgreen'},
                 fliersize=0, linewidth=1)
...
ax.set_ylim((0.001, 2))
ax.set_xlim((-0.5, len(list_gla_nb)-0.5))
ax.text(0.5, -0.175, '(b)', transform=ax.transAxes, ha='center', va='top', fontsize=15,
         zorder=20)
...
ax.text(0.8, -0.085, '(c)', transform=ax.transAxes, ha='center', va='top', fontsize=15,
         zorder=20)

plt.savefig('/home/atom/ongoing/work_stderr_dem/figures/final/Figure_7_final.pdf', dpi=400, transparent=True)
