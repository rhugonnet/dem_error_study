...
def add_submap(fig, slices_grid, array, cmap, col_bounds, label, pos_colorbar=None, add_colorbar=True, label_colorbar=None, add_panel_letter=None, labelpad=None):
    ...
    ax0.text(0.5, 1.025, label, transform=ax0.transAxes, ha='center', va='bottom', fontweight='bold', fontsize=12, zorder=20)
    ...
    if add_panel_letter is not None:
        if labelpad is None:
            ...
            pad = labelpad
        ax0.text(1, -0.4 + pad, add_panel_letter, transform=ax0.transAxes, ha='center', va='top', fontsize=15,
                zorder=20)

    if pos_colorbar is None:
        pos = [-0.5, -0.075, 1, 0.05]
    ...
    if add_colorbar:
        ...
        cb.set_label(label_colorbar, labelpad=-0.5)
...
sns.boxplot(ax=ax, x="slope_category", y="err_northeast", hue="run", hue_order=names,
                 ...
                 fliersize=0, linewidth=1)
ax.vlines(np.arange(0.5, len(bins_slope)-1), ymin=-1, ymax=2, colors='tab:gray', linestyles='dashed', linewidths=0.75)
ax.set_ylim((-0.05, 1.05))
l = ax.legend()
l.remove()

# ax.text(0.025, 0.96, 'd', transform=ax.transAxes, ha='left', va='top', fontweight='bold', fontsize=15,
#          zorder=20)
...
ax.set_xlim((-0.5, len(bins_slope)-1.5))
ax.text(0.5, -0.21, '(c)', transform=ax.transAxes, ha='center', va='top', fontsize=15,
         zorder=20)
...
add_submap(fig, slices_grid=(slice(1, 9), slice(0, 8)), array=hs.data[0, :, :], cmap=cmap, col_bounds=col_bounds, label='$$\\textbf{Hillshade}$$',
           add_colorbar=False)

cmap = plt.get_cmap('Reds', 100)
col_bounds = np.array([0., 20., 40., 60., 80.])
add_submap(fig, slices_grid=(slice(1, 9), slice(8, 16)), array=slope, cmap=cmap, col_bounds=col_bounds, label='$$\\textbf{Slope}$$',
...
add_submap(fig, slices_grid=(slice(11, 19), slice(0, 8)), array=np.cos(aspect * np.pi / 180), cmap=cmap, col_bounds=col_bounds,
           label='$$\\textbf{Northness}$$', add_colorbar=False, add_panel_letter='(a)')
add_submap(fig, slices_grid=(slice(11, 19), slice(8, 16)), array=np.sin(aspect * np.pi / 180), cmap=cmap, col_bounds=col_bounds,
           label_colorbar='Northess or Eastness', label='$$\\textbf{Eastness}$$')
...
add_submap(fig, slices_grid=(slice(22, 30), slice(0, 8)), array=list_slope_map[0], cmap=cmap, col_bounds=col_bounds,
           label='$$\\textbf{Homosc.,}$$'+'\n'+'$$\\textbf{no corr.}$$', add_colorbar=False)

add_submap(fig, slices_grid=(slice(22, 30), slice(8, 16)), array=list_slope_map[5], cmap=cmap, col_bounds=col_bounds,
           label_colorbar='Uncertainty in slope (1$\sigma$, degrees)', label='$$\\textbf{Heterosc.,}$$'+'\n'+'$$\\textbf{long-range}$$')

col_bounds = np.array([0., 0.25,  0.5])
cmap = plt.get_cmap('Greens')
add_submap(fig, slices_grid=(slice(34, 42), slice(0, 8)), array=list_maxnortheast_map[0], cmap=cmap, col_bounds=col_bounds,
           label='$$\\textbf{Homosc.,}$$'+'\n'+'$$\\textbf{no corr.}$$', add_colorbar=False, add_panel_letter='(b)', labelpad=-0.2)

add_submap(fig, slices_grid=(slice(34, 42), slice(8, 16)), array=list_maxnortheast_map[5], cmap=cmap, col_bounds=col_bounds,
           label_colorbar='Maximum uncertainty in\nnorthness or eastness (1$\sigma$)', label='$$\\textbf{Heterosc.,}$$'+'\n'+'$$\\textbf{long-range}$$')

# Save to file
plt.savefig('/home/atom/ongoing/work_stderr_dem/figures/final/Figure_6_final.pdf', dpi=400, transparent=True)
