"""Plotting of Figure S7: heteroscedasticity with curvature and quality of stereo-correlation for the NPI case study"""
import numpy as np
import pandas as pd
import xdem
...
slope, planc, profc = xdem.terrain.get_terrain_attribute(ref_dem, attribute=['slope', 'planform_curvature',
                                                                                'profile_curvature'])
slope = np.maximum(np.abs(planc), np.abs(profc))
...
bins_slope = [0, 0.2, 0.5, 1, 2, 3, 4, 6, 10, 20, 50]
...
for i in range(len(bins_slope) - 1):
    ...
    subset = np.logical_and(slope >= bins_slope[i], slope < bins_slope[i + 1])
...
df = xdem.spatialstats.nd_binning(pleia_ddem.data.ravel(), list_var=[slope.ravel()], list_var_names=['slope'])
...
for i in range(len(bins_slope)-1):
    # Subset by slope category
    subset = np.logical_and(slope.ravel() >= bins_slope[i], slope.ravel() < bins_slope[i+1])
    ...
    if i <= 2:
        bin_name = '{:.1f}'.format(bins_slope[i]) + '–\n' + '{:.1f}'.format(bins_slope[i + 1])
    elif len(bins_curv) - i - 1 <= 3:
        bin_name = '{:.0f}'.format(bins_slope[i]) + '–\n' + '{:.0f}'.format(bins_slope[i + 1])
    else:
        bin_name = '{:.0f}'.format(bins_slope[i]) + '–' + '{:.0f}'.format(bins_slope[i + 1])
...
col_bounds = np.array([5, 10, 20, 30, 50, 80])
cb = []
...
slope_sta = slope[~mask_glacier]
maxabsc_sta = maxabsc[~mask_glacier]

pleia_ddem_gla = pleia_ddem.data[mask_glacier]
slope_gla = slope[mask_glacier]
maxabsc_gla = maxabsc[mask_glacier]

df_sub_all = xdem.spatialstats.nd_binning(pleia_ddem.data, list_var=[slope, maxabsc], list_var_names=['slope', 'maxc'], list_var_bins=(bins_slope, bins_curv))
...
label_var_name_1 = 'Maximum curvature categories (10$^{2}$ m$^{-1}$)'
...
norm = colors.Normalize(vmin=5, vmax=80)
sm = plt.cm.ScalarMappable(cmap=cmap_cus, norm=norm)
sm.set_array([])
cb = plt.colorbar(sm, cax=cbaxes, ticks=[5, 10, 20, 50, 80], orientation='vertical', extend='both', shrink=0.2)
...
plt.savefig('/home/atom/ongoing/work_stderr_dem/figures/final/Figure_S7_final.png', dpi=300)
